...
let randomPaletteOption = null;

// Маппинг ID фракталов на индексы шейдера
const SHADER_TYPE_MAP = {
    'mandelbrot': 0, 'julia': 1, 'burning': 2, 'multibrot': 3,
    'tricorn': 4, 'celtic': 5, 'biomorph': 6, 'newton': 7
};
...
function renderFractalRow() {
    ...
    Object.keys(fractals).forEach(key => {
        ...
        item.textContent = fractals[key];
        item.onclick = () => selectFractal(key);
        container.appendChild(item);
    });
}
...
function selectSystem(key) {
    ...
    renderSystemRow(); renderTypeRow(); renderFractalRow();
}

function selectType(key) {
    state.type = key;
    state.fractal = Object.keys(MENU_DATA[state.system].types[key].fractals)[0];
    renderTypeRow(); renderFractalRow();
}
...
function generateRandomPalette() {
    if (randomPaletteOption) selectPalette.removeChild(randomPaletteOption);
    ...
    randomPaletteOption.value = "random";    randomPaletteOption.textContent = name;
...
function setInputMode(mode) {
    ...
    seedInput.placeholder = mode === 'text' ? "Введите вопрос..." : "Введите сид...";
}

function setDateSeed() { seedInput.value = Date.now().toString(); state.seedValue = seedInput.value; }
function setRandomSeed() { 
    seedInput.value = (Math.floor(Math.random() * 9000000000) + 1000000000).toString(); 
...
function setupInputHandlers() {
    glCanvas.addEventListener('mousedown', (e) => {
        ...
        isDragging = true; lastMouse = [e.clientX, e.clientY];
        glCanvas.style.cursor = 'grabbing';
    });
    window.addEventListener('mouseup', () => { isDragging = false; glCanvas.style.cursor = 'move'; });
    window.addEventListener('mousemove', (e) => {
        ...
        viewCenter[0] -= dx * scale; viewCenter[1] += dy * scale;
        lastMouse = [e.clientX, e.clientY];
        if (gl && prog) renderWebGL();
    });
    glCanvas.addEventListener('wheel', (e) => {
        if (state.system !== 'escape') return;        e.preventDefault();
        const newZoom = viewZoom * Math.exp(-e.deltaY * 0.001);
    ...
    glCanvas.addEventListener('touchstart', (e) => {
        ...
        if (e.touches.length === 2) {
            ...
            initialPinchDist = Math.sqrt(dx*dx + dy*dy); initialZoom = viewZoom;
        } else if (e.touches.length === 1) { lastTouchPos = [e.touches[0].clientX, e.touches[0].clientY]; }
    }, { passive: true });
    ...
    glCanvas.addEventListener('touchmove', (e) => {
        ...
        if (e.touches.length === 2 && initialPinchDist) {
            ...
            const dx = e.touches[0].clientX - e.touches[1].clientX;
            const dy = e.touches[0].clientY - e.touches[1].clientY;
            viewZoom = Math.max(0.5, Math.min(1000000, initialZoom * (Math.sqrt(dx*dx+dy*dy) / initialPinchDist)));
            if (gl && prog) renderWebGL();
...
function calculateSeedParams() {
    ...
    if (state.fractal === 'julia' || state.fractal === 'biomorph') {
...
function generate() {
    try {
        ...
        if (state.system === 'escape') {
            glCanvas.style.display = 'block'; attrCanvas.style.display = 'none';
            initWebGL(); renderWebGL();
        } else if (state.system === 'chaos') {
            glCanvas.style.display = 'none'; attrCanvas.style.display = 'block';
            renderAttractor();
        } else {
            alert("Geometric режим в разработке. Скоро появятся L-системы!");
        }
        document.getElementById('panel').classList.remove('open');
    } catch (e) { showError(e.message); }
}

function showError(msg) { errorText.textContent = msg; errorBox.style.display = 'block'; console.error(msg); }
...
function initWebGL() {
    ...
    gl.shaderSource(vsObj, vertexShaderSource); gl.compileShader(vsObj);
    const fsObj = gl.createShader(gl.FRAGMENT_SHADER);
    gl.shaderSource(fsObj, fragmentShaderSource); gl.compileShader(fsObj);

    if (!gl.getShaderParameter(fsObj, gl.COMPILE_STATUS)) {
        const log = gl.getShaderInfoLog(fsObj); gl.deleteShader(fsObj);
    ...
    uniformLocs = {
        res: gl.getUniformLocation(prog, 'u_res'), zoom: gl.getUniformLocation(prog, 'u_zoom'),
        center: gl.getUniformLocation(prog, 'u_center'), cx: gl.getUniformLocation(prog, 'u_cx'),
        cy: gl.getUniformLocation(prog, 'u_cy'), type: gl.getUniformLocation(prog, 'u_type'),
...
function renderWebGL() {
    ...
    // Маппинг ID на индекс шейдера
    gl.uniform1i(uniformLocs.type, SHADER_TYPE_MAP[state.fractal] || 0);

    // Обработка палитры (рандом = индекс 9)
    gl.uniform1i(uniformLocs.palette, state.palette === "random" ? 9 : parseInt(state.palette));

    gl.drawArrays(gl.TRIANGLE_STRIP, 0, 4);
}

function renderAttractor() {
    const w = attrCanvas.width, h = attrCanvas.height;
    ctxAttr.fillStyle = '#050505'; ctxAttr.fillRect(0, 0, w, h);
    let seedNum = parseFloat(state.seedValue.replace(/\D/g, '')) || 123456789;
    let a = Math.sin(seedNum * 0.0001) * 2.5, b = Math.cos(seedNum * 0.00013) * 2.5;
    let c = Math.sin(seedNum * 0.00017) * 2.5, d = Math.cos(seedNum * 0.00019) * 2.5;
    let x = 0.1, y = 0.1;
    ...
    for (let i = 0; i < skip; i++) {
        let nx, ny;
        if (state.fractal === 'clifford') { nx = Math.sin(a*y) + c*Math.cos(a*x); ny = Math.sin(b*x) + d*Math.cos(b*y); }
        else if (state.fractal === 'dejong') { nx = Math.sin(a*y) - Math.cos(b*x); ny = Math.sin(c*x) - Math.cos(d*y); }
        else if (state.fractal === 'hopalong') { let x_old = x; x = y - Math.sign(x_old)*Math.sqrt(Math.abs(b*x_old-c)); y = a - x_old; continue; }
        else { nx = Math.sin(b*y) + c*Math.sin(b*x); ny = Math.sin(a*x) + d*Math.sin(a*y); }
        x = nx; y = ny;
    }
    for (let i = 0; i < iter; i++) {
        let nx, ny;        if (state.fractal === 'clifford') { nx = Math.sin(a*y) + c*Math.cos(a*x); ny = Math.sin(b*x) + d*Math.cos(b*y); }
        else if (state.fractal === 'dejong') { nx = Math.sin(a*y) - Math.cos(b*x); ny = Math.sin(c*x) - Math.cos(d*y); }
        else if (state.fractal === 'hopalong') { let x_old = x; x = y - Math.sign(x_old)*Math.sqrt(Math.abs(b*x_old-c)); y = a - x_old; nx = x; ny = y; }
        else { nx = Math.sin(b*y) + c*Math.sin(b*x); ny = Math.sin(a*x) + d*Math.sin(a*y); }
        if (state.fractal !== 'hopalong') { x = nx; y = ny; }
        let scale = state.fractal === 'hopalong' ? 0.08 : 0.15;
        let px = (x * scale + 0.5) * w, py = (y * scale + 0.5) * h;
        if (px > 0 && px < w && py > 0 && py < h) ctxAttr.fillRect(px, py, state.fractal === 'hopalong' ? 2.0 : 1.5, state.fractal === 'hopalong' ? 2.0 : 1.5);
    }
}

function getAttractorColor() {
    const p = state.palette;
    if(p === "random" || p == 2) return `rgba(255, 0, 255, 0.1)`;
...
function savePNG() {
    let dataUrl = state.system === 'escape' ? glCanvas.toDataURL('image/png') : attrCanvas.toDataURL('image/png');
